				...
				<c:if test="${!empty glist}">
					<c:forEach var="g" items="${glist}">
						<tr>
							...
							<td><a
								href="admin_gongji_cont?no=${g.gno}&page=${page}&state=cont">
									<c:if test="${fn:length(g.gtitle)>16}">
...
${g.gtitle}
							...
							<td align="center">${g.gname}</td>
							<td align="center">${fn:substring(g.gdate,0,10)}</td>
							<td align="center">${g.ghit}</td>
							<td align="center"><input type="button" value="수정"
								onclick="location=
'admin_gongji_cont?no=${g.gno}&page=${page}&state=edit';" />
								<input type="button" value="삭제"
								onclick="if(confirm('정말로 삭제할까요?') == true){
location=
'admin_gongji_del?no=${g.gno}&page=${page}';	
}else{ return ;}" />
			...
			<div id="bFind_wrap">
				<select name="find_field">
					<option value="gongji_title"
						<c:if test="${find_field=='gtitle'}">
   ${'selected'}</c:if>>제목</option>
					<option value="gongji_cont"
						<c:if test="${find_field=='gcont'}">
