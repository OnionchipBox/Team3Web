	...
	public String kakaopay() {
		...
		try {
			...
			conn.setDoOutput(true); // 연결을 통해서 서버에게 전해줄 것이 있을 때 true (DoInput은 기본적으로 true로 되어 있고, DoOutput은 기본 설정이 false로 되어 있어서 변경해주는 것)
						...
						//전해줄 파라미터 
			String param ="cid=TC0ONETIME&partner_order_id=partner_order_id&partner_user_id=partner_user_id&item_name=초코파이&quantity=1&total_amount=2200&vat_amount=200&tax_free_amount=0&approval_url=http://localhost:7996/cls/jq/pay.cls&fail_url=https://localhost:7996/fail&cancel_url=https://localhost:7996/cancel";
