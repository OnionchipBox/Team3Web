...
create table shop_gongji(
  ...
  ALTER TABLE shop_gongji MODIFY (gcont VARCHAR2(4000));
  ...
  insert into shop_gongji (gno, gname, gtitle, gpwd, gcont, gdate)
  values(gongji_no_seq.nextval, 'NUBE관리자','NUBE DRAW - 나이키 후드티','admin','공지내용입니다.',sysdate);
  
  select * from shop_gongji order by gno desc;
